...
from player_actions.unequip import Unequip
...
def run_command(game, command, arguments):
    ...
    elif command == 'equip':  # done
        return equip(game, arguments)

    elif command in ['go', 'walk', 'move', 'visit']: # done
...
def attack(game, arguments):
    ...
    if valid:
        thing_hit, is_dead, message = Attack.act(
            game, target, is_creature, weapons)

    # Allow creature to attack back
    if is_creature and not is_dead:
        target = 'game_player'
        weapons = thing_hit.choose_weapons()
        thing_hit, is_dead, additional_message = Attack.act(
            game, target, is_creature, weapons)
        message += '\n' + additional_message
...
def unequip(game, arguments):
    target_item, _ = Unequip.parse(game, arguments)
    valid, message = Unequip.can_act(game, target_item)
    if valid:
        message = Unequip.act(game, target_item)
    return GameResponse(game.player.id, message, None)
